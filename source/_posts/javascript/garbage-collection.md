...
程序在运行过程中，已经不再需要使用的内存，没有被操作系统或垃圾回收机制正确释放，导致 **可用内存持续减少** 的现象即 {% label 内存泄露 blue %}。
...
{% label 分代回收机制 pink %} 把堆内存分为新生代（存放短生命周期对象）和老生代（存放长生命周期/大对象），针对两个区域的对象特性，使用不同的回收算法，最大化 GC 效率。
